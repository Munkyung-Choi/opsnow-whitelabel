...
4. **Session Wrap — 일지 작성 안전망**: 사용자가 "세션 종료", "오늘 마무리", "내일 보자", "정리" 등 세션 종료 신호를 보내면 오늘 날짜의 `docs/journal/YYYY-MM-DD.md` 파일 존재 여부를 확인한다. 없거나 오늘 세션의 주요 결정이 반영되지 않은 경우 **`/end-dev` 스킬 실행을 자동 제안**한다. 이는 비개발자 오너의 "이해도 부채" 방지 핵심 수단으로, 누락하지 않는다.
...
외부 LLM(ChatGPT 등)이 로직·보안·DB 등 **기능 변경** 제안 시: ① 자체 검증(동의·반박 정리) → ② 문경 님 보고 → ③ 승인 후 구현. LOW 트랙(텍스트·CSS·오탈자)은 일반 절차. 외부 피드백은 **참고 자료**이며 **구현 지시가 아니다**.
