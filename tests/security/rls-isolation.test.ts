...
// =============================================================================
describe('leads_masked_view — 마스킹 접근 제어 검증 (WL-138 Scenario C)', () => {
  it('[access-control] partner_admin은 leads_masked_view를 조회할 수 없다 (0건)', async () => {
    // security_invoker=true 하에서:
    //   partner_a가 leads에서 얻는 rows → leads_partner_admin_select 정책으로 자기 파트너 rows
    //   그 rows를 view의 WHERE EXISTS(role='master_admin')로 필터 → 0건
    const { data, error } = await partnerAClient
  ...
  it('[design-gap / DEBT-007] master_admin도 leads_masked_view에서 현재 0건 반환 — view 접근 경로 미완성', async () => {
    // ⚠️ 설계 갭: leads_masked_view는 security_invoker=true + master_admin의 leads SELECT 정책 부재로
    //   master_admin에게도 0건을 반환한다. leads_partner_admin_select는 owner_id=auth.uid()
    //   조건을 쓰므로 master_admin(어느 파트너의 owner도 아님)은 RLS를 통과하지 못함.
    //   수정 방향(DEBT-007): view를 SECURITY DEFINER로 변경하거나 master_admin SELECT 정책 추가.
    const { data, error } = await masterClient
      .from('leads_masked_view')
      .select('email')
      .eq('partner_id', PARTNER_B.partnerId)
      .limit(1)
    expect(error).toBeNull()
    expect(data).toHaveLength(0) // 현재 실제 동작 — 갭 문서화
  })

  it.todo('[intended-design] master_admin이 leads_masked_view에서 마스킹된 이메일(***@domain)을 조회할 수 있어야 함 — DEBT-007 상환 후 활성화')
})
