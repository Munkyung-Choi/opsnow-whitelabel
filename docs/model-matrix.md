...
| 데이터 | DB 스키마·마이그레이션 (HIGH-Critical: Type 변경·Data Backfill) | **Opus** | 취소 불가 작업의 무결성 보장 |
| 데이터 | DB 스키마·마이그레이션 (HIGH-Standard: nullable Column·Index 추가) | **Opus** | HIGH 트랙이므로 Opus 유지, 승인은 비동기 |
